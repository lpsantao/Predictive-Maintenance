...
total_cycles = 251+38# 448 = 56*8blocks of 3H - 56 days - 8 weeks
current_cycle = 0
...
while (current_cycle < total_cycles): # total_cycles):
    ...
    #         if current_cycle <271:
    #     #print(current_simul_date, current_cycle, week_comp)
    #             load = np.array([None, 2,1,0])
    #     if current_cycle >270:
    #         if current_cycle < 279:
    #             load = np.array([None, 1, 1, 0])
    #     if current_cycle >278:
    #         load = np.array([None, 0, 0, 0])

    if current_simul_date[1] > pd.to_datetime('2015-12-01 03:00:00'):
        ll.append(load[1])
        dff = pd.DataFrame({'col': ll})
    if current_simul_date[2] > pd.to_datetime('2015-12-01 03:00:00'):
        ll2.append(load[2])
        dff2 = pd.DataFrame({'col': ll2})
    if current_simul_date[3] > pd.to_datetime('2015-12-01 03:00:00'):
        ll3.append(load[3])
    ...
    # sys.exit("END")
    current_cycle +=1
#    cena = pd.to_datetime(X.loc[(X['machineID'] == 3) & (X['failure'] == 'comp2'), ['datetime']].values[7])
#    cena2 = pd.to_datetime(X.loc[(X['machineID'] == 1) & (X['failure'] == 'comp2'), ['datetime']].values[7])

with sns.axes_style("darkgrid"):
    ax1 = plt.subplot(211)
    plot_df = X.loc[(X['machineID'] == 1), ['datetime', 'rotatemean_3h']]
    plot_df2 = plot_df.loc[(plot_df['datetime'] > pd.to_datetime('2015-12-01 0:00:00'))]
    x2 = plot_df2['datetime']
    ax2 = ax1.twinx()
    ax1.yaxis.grid(False)
    color = 'C1'
    plt.axvline(x= pd.to_datetime('2015-12-02 09:00:00'), linestyle='dashed')
    plt.axvline(x= pd.to_datetime('2015-12-05 18:00:00'), linestyle='dashed')
...
with sns.axes_style("darkgrid"):
    ...
    plot_df21 = plot_df1.loc[(plot_df1['datetime'] > pd.to_datetime('2015-12-01 0:00:00'))]
