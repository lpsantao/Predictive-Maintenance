...
total_cycles = 448#95# 448 = 56*8blocks of 3H - 56 days - 8 weeks
current_cycle = 0
...
# fixed variables:
    ...
    total_mach = 3
    mttr = np.array([None, 9, 9, 9, 9, 9])  # mean time to repair per machine ID (defined)
    mtbf = np.array([[None, 4320 * 1.06, 3408 * 1.06, 1800 * 1.06, 2160 * 1.06, 3240 * 1.06, 2640 * 1.2, 1080 * 1.2,
                      1080 * 1.2,  # load -2
                      1200 * 1.2, 2160 * 1.2, 3240 * 1.2, 1980 * 1.2, 4320 * 1.2, 4317 * 1.2, 3418 * 1.2, 900 * 1.2],
                     [None, 4320 * 1.03, 3408 * 1.03, 1800 * 1.03, 2160 * 1.03, 3240 * 1.03, 2640 * 1.03, 1080 * 1.03,
                      1080 * 1.06,
                      1200 * 1.06, 2160 * 1.06, 3240 * 1.06, 1980 * 1.06, 4320 * 1.06, 4317 * 1.06, 3418 * 1.06, 900 * 1.06],
                     # load -1
                     [None, 4320, 3408, 1800, 2160, 3240, 2640, 1080, 1080, 1200, 160, 3240, 1980, 4320, 4317, 3418,
                      900],  # load 0
                     [None, 4320 / 1.03, 3408 / 1.03, 1800 / 1.03, 2160 / 1.03, 3240 / 1.03, 2640 / 1.03, 1080 / 1.03,
                      1080 / 1.5, 1200 / 5,
                      2160 / 5, 3240 / 5, 1980 / 5, 4320 / 5, 4317 / 5, 3418 / 5, 900 / 5],  # load +1
                     [None, 4320 / 1.06, 3408 / 1.06, 1800 / 1.06, 2160 / 1.06, 3240 / 1.06, 2640 / 2, 1080 / 2, 1080 / 2, 1200 / 2,
                      2160 / 10, 3240 / 10, 1980 / 10,
                      4320 / 10, 4317 / 10, 3418 / 10, 900 / 10],  # load +2
                     ])
    last_fail = np.array([0,(current_simul_date1[2]+pd.to_timedelta(3,unit='D'))-pd.to_timedelta((mtbf[2][2])/24, unit='D'),(current_simul_date1[2]+pd.to_timedelta(3,unit='D')+ pd.to_timedelta(12,unit='H'))-pd.to_timedelta((mtbf[2][3])/24, unit='D')])
...
# change each call
    ...
    next_week_last = current_simul_date1[1] + pd.to_timedelta(size, unit='D')+ pd.to_timedelta(3, unit='D') # last date of the 3 days of next week

# Calculates next fail date
    ...
    for mach in range(0, total_mach):
        if mach == (id_failed1 - 1):
            a[mach] = current_simul_date1[id_failed1] + pd.to_timedelta(21, unit='H')
            next_fail_time[mach] = a[mach]
        else:
            a[mach] = last_fail[mach] + pd.to_timedelta((mtbf[(individual[(size * mach)]) + 2][mach + 1]), unit='H')
...
# Calculates production
    for ID in range(0, total_mach):
        ...
        while j < (((size - 1) + (ID * size))) + 1:
            ...
            print('mblock, stopblock, fail block, afterblck, m, tmf', blocks_maint, block_stop, blocks_fail , block_after, m, time_in_fail)
            #print('bf, ba,bm, bs', blocks_fail, block_after, blocks_maint, m, block_stop)
            j += 1
            i += 1
            print(prod1)
    fit = (target[0]-prod1)**2 + ((sum(fail_in)) * 900) + (fail_out * 600) + (sdev * 800) + (change * 500) + (up * 50)
    if target[0]-prod1>0:
        fit=1000000
    print('fit',fit)
    return fit,
...
def flag3 (total_mach, mean_fail, std_fail, X, current_simul_date, ID, index, prediction, flag_state):
...
while (current_cycle < total_cycles): # total_cycles):
    ...
    for ID in range(1, total_mach + 1):
        if flag_state[ID] == 0: #if real data
            ...
            X, index, current_simul_date, flag_state= flag3(total_mach, mean_fail, std_fail, X, current_simul_date, ID, index, prediction, flag_state)
    # calls predictor
    ...
    if current_simul_date[1] > pd.to_datetime('2015-11-01 3:00:00'):
        ll.append(load[1])
        dff = pd.DataFrame({'col': ll})
    if current_simul_date[2] > pd.to_datetime('2015-11-01 3:00:00'):
        ll2.append(load[2])
        dff2 = pd.DataFrame({'col': ll2})
    if current_simul_date[3] > pd.to_datetime('2015-11-01 3:00:00'):
        ll3.append(load[3])
        dff3 = pd.DataFrame({'col': ll3})
...
# X.to_csv('simulated.csv', index=False)
cena = pd.to_datetime(X.loc[(X['machineID'] == 1) & (X['failure'] == 'comp2'), ['datetime']].values[7])
cena2 = pd.to_datetime(X.loc[(X['machineID'] == 2) & (X['failure'] == 'comp2'), ['datetime']].values[7])
cena3 = pd.to_datetime(X.loc[(X['machineID'] == 3) & (X['failure'] == 'comp2'), ['datetime']].values[7])

with sns.axes_style("darkgrid"):
    ax1 = plt.subplot(211)
    plot_df = X.loc[(X['machineID'] == 1), ['datetime', 'rotatemean_3h']]
    plot_df2 = plot_df.loc[(plot_df['datetime'] > pd.to_datetime('2015-11-01 00:00:00'))]
    x2 = plot_df2['datetime']
    ax2 = ax1.twinx()
    ax1.yaxis.grid(False)
    color = 'C1'
    plt.axvline(x= pd.to_datetime('2015-11-03 03:00:00'), linestyle='dashed')
    plt.axvline(x= pd.to_datetime('2015-11-08 00:00:00'), linestyle='dashed')
    ax2.set_ylabel('Throughput Rate', color=color)  # we already handled the x-label with ax1
    ax2.plot(x2, dff['col'], color=color)
    ax2.set_ylim(-2.5,2.5)
    ax2.tick_params(axis='y', labelcolor=color)
    ax1.set_ylabel('Rotate 3H mean Machine 1', color='b')
    ax1.plot(x2,plot_df2['rotatemean_3h'], color = 'b', linewidth=2.0)
    ax1.tick_params(axis='y', labelcolor='b')
    ax1.set_ylim(-0.5,550)
    # make x-axis ticks legible
    adf = plt.gca().get_xaxis().get_major_formatter()
    adf.scaled[1.0] = '%m-%d'
    plt.xlabel('Date')
...
with sns.axes_style("darkgrid"):
    ...
    plot_df1 = X.loc[(X['machineID'] == 2), ['datetime', 'rotatemean_3h']]
    plot_df21 = plot_df1.loc[(plot_df1['datetime'] > pd.to_datetime('2015-11-01 00:00:00'))]
    x21 = plot_df21['datetime']
    ax12.set_ylabel('Rotate 3H mean Machine 2', color='b')
    ax12.plot(x21,plot_df21['rotatemean_3h'], color = 'b',linewidth=2.0)
    ax12.tick_params(axis='y', labelcolor='b')
    ax12.set_ylim(-0.5,560)
    ax22 = ax12.twinx()
    ax12.yaxis.grid(False)
    color = 'C1'
    plt.axvline(x= pd.to_datetime('2015-11-03 03:00:00'), linestyle='dashed')
    plt.axvline(x= pd.to_datetime('2015-11-08 00:00:00'), linestyle='dashed')
    ax22.set_ylabel('Throughput Rate', color=color)  # we already handled the x-label with ax1
    ax22.plot(x21, dff2['col'], color=color)
...
with sns.axes_style("darkgrid"):
    ax1 = plt.subplot(211)
    plot_df = X.loc[(X['machineID'] == 3), ['datetime', 'rotatemean_3h']]
    plot_df2 = plot_df.loc[(plot_df['datetime'] > pd.to_datetime('2015-11-01 00:00:00'))]
    x2 = plot_df2['datetime']
    ax1.set_ylabel('Rotate 3H mean Machine 3', color='b')
    ax1.plot(x2,plot_df2['rotatemean_3h'], color = 'b', linewidth=2.0)
    ax1.tick_params(axis='y', labelcolor='b')
    ax1.set_ylim(-0.5,560)
    ax2 = ax1.twinx()
    ax1.yaxis.grid(False)
    color = 'C1'
    plt.axvline(x= pd.to_datetime('2015-11-03 03:00:00'), linestyle='dashed')
    plt.axvline(x= pd.to_datetime('2015-11-08 00:00:00'), linestyle='dashed')
    ax2.set_ylabel('Throughput Rate', color=color)  # we already handled the x-label with ax1
    ax2.plot(x2, dff3['col'], color=color)
    ax2.set_ylim(-2.5,2.5)
    ax2.tick_params(axis='y', labelcolor=color)
