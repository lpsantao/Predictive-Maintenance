...
total_cycles = (8*10)+15 # 448 = 56*8blocks of 3H - 56 days - 8 weeks
current_cycle = 0
...
while (current_cycle < total_cycles): # total_cycles):
    ...
    if start_ga == 1:
        if current_cycle > 17:
            if current_cycle < 25:
                # print(current_simul_date, current_cycle, week_comp)
                load = np.array([None, 2, -1, -1])
        if current_cycle > 24:
            if current_cycle < 55:
                load = np.array([None, 2, -1, 1])
        if current_cycle > 55:
            load = np.array([None, 0, 0, 0])
